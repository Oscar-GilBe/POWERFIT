...
<head>
    ...
    <title>Actividades - Modificar - Powerfit</title> <!-- Título de la página -->
     ...
     <!-- Encabezado -->
        ...
        <section>
            ...
            <div class="title-container">
                <h1 class="page-title">MODIFICAR ACTIVIDAD</h1>
                <p class="page-subtitle">Aquí podrás modificar los detalles de la actividad existente, y sus clases para el próximo mes.</p>
            </div>
           
            <div class="container">
			<!-- Formulario para modificar una actividad existente -->
        	<form action="/POWERFIT/administrador/ModificarActividad" method="post" enctype="multipart/form-data" class="form-container">
        	
            <!-- Nombre de la actividad (solo lectura) -->
            <div class="form-group">
                <label for="nombre">Nombre (no es modificable):</label>
            ...
            <div class="form-group">
                ...
                <input type="number" id="plazas_ofertadas" name="plazas_ofertadas" min="1" value="${actividad.plazas}" required>
            </div>
			...
			<c:forEach var="day" items="${dias}">
			    <div class="day-row">
			        <!-- Checkbox de activación para cada día -->
			        <input type="checkbox" id="${day}-checkbox" name="dias[]" value="${day}" 
			               <c:if test="${horariosPorDia[day] != null}">checked</c:if>
			               onchange="toggleDayTimes('${day}')">
			        <label for="${day}-checkbox">${day}</label>
			        
			        <!-- Grupo de horarios para el día actual -->
			        <div id="${day}-times" class="hours-group" style="display: ${horariosPorDia[day] != null ? 'flex' : 'none'};">
			            <c:forEach var="horario" items="${horariosPorDia[day]}">
			                <div class="time-row">
			                    <!-- Horario de inicio y fin prellenado -->
			                    <label>Inicio:</label>
			                    <input type="time" name="${day}_inicio[]" 
			                           value="${fn:substringBefore(horario, ' - ')}"
			                           onchange="updateEndTime(this)" step="3600" min="08:00" max="19:00">
			                    <label>Fin:</label>
			                    <input type="time" name="${day}_fin[]" 
			                           value="${fn:substringAfter(horario, ' - ')}" readonly>
			                </div>
			            </c:forEach>
			            
			            <!-- Botón para añadir más horarios -->
            ...
            <div class="form-group">
                ...
                <div class="difficulty">
	                <input type="radio" id="facil" name="dificultad" value="FACIL" ${actividad.dificultad == 'FACIL' ? 'checked' : ''} required>
	                <label for="facil">Fácil</label>
	                
	                <input type="radio" id="media" name="dificultad" value="MEDIA" ${actividad.dificultad == 'MEDIA' ? 'checked' : ''} required>
	                <label for="intermedio">Media</label>
	                
	                <input type="radio" id="dificil" name="dificultad" value="DIFICIL" ${actividad.dificultad == 'DIFICIL' ? 'checked' : ''} required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="material" name="material" value="${actividad.material}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="sala_asignada" name="sala_asignada" required>
                    ...
                    <c:forEach var="sala" items="${listaSalas}">
                        <option value="${sala.numero}" ${actividad.sala == sala.numero ? 'selected' : ''}>${sala.numero}</option>
                    </c:forEach>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="grupo_muscular" name="grupo_muscular"  value="${actividad.grupo_muscular}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="monitor" name="monitor" required>
                    ...
                    <c:forEach var="monitor" items="${listaMonitores}">
                        <option value="${monitor.correo}" ${actividad.monitor == monitor.correo ? 'selected' : ''}>${monitor.correo}</option>
                    </c:forEach>
                </select>
            </div>
             ...
             <!-- Fecha límite -->
            <div class="form-group">
                <p>Fecha límite: Se actualizarán las clases para el siguiente mes.</p>
            </div>
            ...
            <button type="submit" class="submit-btn">Modificar Actividad</button>
        </form>
